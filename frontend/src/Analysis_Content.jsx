...
import {
  CloudUpload, FileText, Hash, ListChecks,
...
export default function AnalysisContent() {
    ...
    setTimeout(() => {
      const dummyResult = {
        ...
        feedback: {
          ...
          '논리성': '초반 주장과 후반 설명 사이에 근거 연결이 약간 부족합니다. 예: "그러나" 이후에 전환이 어색합니다.',
      ...
      setStats(dummyResult.stats);
      setErrors(dummyResult.errors);
      setOriginalText(dummyResult.originalText);
      setCorrectedText(dummyResult.correctedText);
      setHighlightedText(dummyResult.highlightedText);
      setFeedback(dummyResult.feedback);
      setHtmlLink(dummyResult.htmlLink);
      setOpenFeedback(Object.fromEntries(Object.keys(dummyResult.feedback).map(key => [key, true])));
      setLoading(false);
    }, 1500);
  };
  ...
  const chartData = Object.entries(feedback).map(([key]) => ({ category: key, score: 4 + Math.random() }));
    ...
    <div className="container mx-auto p-8 space-y-20">
      ...
      {file && progress < 100 && (
        <div className="max-w-xl mx-auto">
          ...
          <p className="text-sm text-gray-600">맞춤법 검사 중…</p>
        </div>
      )}
      ...
      {progress === 100 && stats && (
        <section className="space-y-10">
          ...
          {/* 오류 테이블 */}
          {errors.length > 0 && (
            <div className="p-4 bg-white border border-gray-100 rounded-lg overflow-auto">
...
function TabButton({ label, tab, activeTab, setActiveTab }) {
  return (
    <button
      ...
      className={`px-4 py-2 rounded ${activeTab === tab ? 'bg-[#6EAED5] text-white' : 'bg-gray-100 text-gray-600'}`}
    >
      {label}
    </button>
  );
